...
describe("Getting Prices from PNG Contracts", function() {
  async function deployFixture() {
    let pngPrices;

    const PNGPrices = await ethers.getContractFactory("PNGPrices");
  ...
  describe.only("Get Pair Reserves", () => {
    it("avax/usdc and avax/usdc.e: getReserves", async () => {
      ...
      const Pair = await pngPrices.getReserves(
        addresses.mainnet.tokens.avax.wavax,
        addresses.mainnet.tokens.avax.usdc
      );
      ...
      const Pair1 = await pngPrices.getReserves(
        addresses.mainnet.tokens.avax.wavax,
        addresses.mainnet.tokens.avax.usdc_e
      );
